...
seagrass_metadata[seagrass_metadata=="initial"]<-"Initial"
seagrass_metadata[seagrass_metadata=="final"]<-"Final"
...
metaasv = inner_join(seagrass_metadata,seagrass_ASV[,-(2:3)],by="SampleID")
metaasv.filtered=subset(metaasv,Treatment=="Control"|Treatment=="GeO2"|Trial=="2")
#set seed for reproducibility
set.seed(4)
#NMDS performed on ASV Filtered (removing the Sample ID column, not needed)
seagrass_ASV_NMDS<-metaasv.filtered[,7:2879] 
#NMDS reduced to 2 dimensions, measured with Bray-Curtis
nmds_s4=metaMDS(seagrass_ASV_NMDS,k=2,distance="bray") 
nmds_s4
#Initial NMDS plot
plot(nmds_s4)
...
#and trial
data.scores$Treatment=as.character(metaasv.filtered$Treatment)
data.scores$Time=as.character(metaasv.filtered$Time)
...
#Creating a new column for and calculating the relative abundances for asvmeta_order data frame
...
#Creating a subset of the metaasv data frame, containing all data for Control treatment or GeO2 treatment
Control=subset(metaasv, metaasv$Treatment == "Control")
ControlASV=Control[7:2879] #removing non-ASV columns
GeO2=subset(metaasv, metaasv$Treatment == "GeO2")
GeO2ASV=GeO2[7:2879] #removing non-ASV columns

#Calculating the species richness of each row
GeO2_Richness=apply(GeO2ASV,1,richness)
Control_Richness=apply(Control,1,richness)
...
ggplot(data.scores, aes(x=NMDS1, y=NMDS2))+
  geom_point(size=3, aes(shape=Treatment, color=Time))+
  theme_classic()+
...
ggplot(GeO2_top_taxa,aes(x=SampleID,y=relative_abundance, fill=Rank5)) +
  ...
  labs(fill="Genus")+theme(legend.text=element_text(face="bold.italic"))+
...
#Statistic 3
#Calculating the mean species richness of the rows
mean(GeO2_Richness)
mean(Control_Richness)
#Mean richness of GeO2 group=268.5 
#Mean richness of control group=130.6
#We can see that the species richness has increased following diatom removal
